...
def plot_runtime(dfs):
    ...
    for df_i, df in enumerate(dfs):
        ...
        runtime.plot(label="fold {} ({:.2f} min/epoch in average)".format(df_i, np.mean(runtime)))
    plt.xlabel("epoch")
    plt.ylabel("time [min]")
    plt.legend()
...
def plot_learning_with_two_scales(df, ylim, just_test=False, out_dir=None):
    ...
    l1 = ax1.plot(mean_loss, color="g",
                  label="train_loss ({:.2f} +-{:.2f})".format(mean_loss_final, std_loss_final))
    ...
    l2 = ax1.plot(mean_loss, color="r",
                  label="test_loss ({:.2f} +-{:.2f})".format(mean_loss_final, std_loss_final))
    ...
    if not just_test:
        for i in range(len(df)):
            ax2.plot(df[i]["train_" + misclass_or_rmse] * factor, linestyle="--", color="b", linewidth=.5, label="")
    mean_misclass = np.mean([df[i]["train_" + misclass_or_rmse] * factor for i in range(len(df))], axis=0)
    mean_misclass_final = np.mean([df[i]["train_" + misclass_or_rmse].iloc[-1] * factor for i in range(len(df))],
                                  axis=0)
    ...
    l3 = ax2.plot(mean_misclass, linestyle="--", color="b",
                  label="train_" + misclass_or_rmse + " ({:.2f} +-{:.2f})".format(mean_misclass_final,
                                                                                  std_misclass_final))

    for i in range(len(df)):
        ax2.plot(df[i]["test_" + misclass_or_rmse] * factor, linestyle="--", color="orange", linewidth=.5, label="")
    ...
    l4 = ax2.plot(mean_misclass, linestyle="--", color="orange",
                  label="test_" + misclass_or_rmse + " ({:.2f} +-{:.2f})".format(mean_misclass_final,
                                                                                 std_misclass_final))
...
def plot_result_overview(result_directory, decoding_tasks, models, metric, fs=20):
    ...
    for model in models:
        for decoding_task in decoding_tasks:
            for decoding_type in decoding_types:
                path = os.path.join(result_directory, model, decoding_task, decoding_type, "resampy0.2.1_clipafter_rejecting/")
                if os.path.exists(path):
                    dfs = df_list_from_dir(path)[:5]
                    misclasses = [d["test_"+misclass_or_rmse].iloc[-1] for d in dfs]
                    for misclass in misclasses:
                        result_df = result_df.append({
                            "model": model,
                            "subset": decoding_type,
                            "task": decoding_task,
                            misclass_or_rmse: misclass*factor}, ignore_index=True)
                    i += 1
    ...
    for decoding_task in decoding_tasks:
        for model in models:
            for decoding_type in decoding_types:
                d = result_df[(result_df.model == model) & (result_df.task == decoding_task) & (result_df.subset == decoding_type)]
                if len(d) > 0:
                    sns.regplot(np.random.rand(len(d))+i+.5, d[misclass_or_rmse], fit_reg=False, color=cm(1.*i/n_colors))
                    sns.regplot(np.array([i+1]), np.array([d[misclass_or_rmse].mean()]), marker="^", fit_reg=False,
                                scatter_kws={'s': 200}, color=cm(1.*i/n_colors),
                               label="{:.2f} ($\pm$ {:.2f})".format(d[misclass_or_rmse].mean(), d[misclass_or_rmse].std()))
